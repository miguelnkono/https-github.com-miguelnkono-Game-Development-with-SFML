...
Textbox::Textbox(int l_visible, int charSize, int l_width, sf::Vector2f l_screenPos)
{
    Setup(l_visible, charSize, l_width, l_screenPos);
}
...
void Textbox::Setup(int l_visible, int charSize, int l_width, sf::Vector2f l_screenPos)
{
    m_numVisible = l_visible;

    sf::Vector2f l_offset(2.0f, 2.0f);

    m_font.loadFromFile("C:\\Users\\Nkono Ndeme Miguel\\Desktop\\SFML-JourneyLearn\\GameDevelopment\\Others\\Fonts\\arialbd.ttf");
    ...
    m_backdrop.setSize(sf::Vector2f(l_width, (l_visible * (charSize * 1.2f))));
